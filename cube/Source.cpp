...
int main()
{
	...
	// Front Face
	for (int i = 0; i <= 8; i++) {
		mySquares.push_back(Square::Square(
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 0; i < mySquares.size(); i++) {
			...
			if (mySquares[i].rotationRemaining > 0) {
				inMotion = true;
				mySquares[i].rotateAround(glm::radians(1/3.0) , origin, mySquares[i].rotationAxis);
				mySquares[i].rotationRemaining -= glm::radians(1/3.0);
			}
			mySquares[i].draw();
		}
		if (!inMotion) {
